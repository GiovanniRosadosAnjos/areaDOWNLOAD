...
- **Efeito-fidelidade/CPF**: no caixa de muitos supermercados, há duas perguntas SEPARADAS: (1) informar o CPF para o programa de fidelidade da loja (pode gerar desconto/pontos internamente), e (2) imprimir esse CPF na nota fiscal. São decisões independentes — o cliente pode informar o CPF para fidelidade e ainda assim recusar a impressão na nota. Por isso, **o campo impresso na nota ("CONSUMIDOR IDENTIFICADO" vs. "CONSUMIDOR NÃO IDENTIFICADO") NÃO é uma fonte confiável** para saber se houve desconto de fidelidade — ele só reflete a decisão de impressão, não a de cadastro. Uma variação de preço pode ser efeito-fidelidade mesmo numa nota que diz "não identificado".

Uma variação no valor total, sozinha, não separa nenhum desses efeitos. Sempre que for falar em "inflação" (variação de preço no tempo), a comparação correta é: **mesmo produto, mesma origem/loja (coluna Origem), valor unitário, em datas diferentes, idealmente com o mesmo status de CPF/fidelidade informado no caixa**. Se a origem mudar entre os dois pontos comparados, isso é um sinal de alerta — a diferença pode ser efeito-loja, não inflação. Da mesma forma, se o status de CPF/fidelidade mudar entre as duas compras comparadas, avise que a diferença pode ser efeito-fidelidade, não inflação.

**Registrando o status de CPF/fidelidade**: como o campo impresso na nota não é confiável, **pergunte diretamente ao usuário** se ele informou o CPF/fidelidade no caixa daquela compra (independente do que aparece impresso) ao lançar uma nota nova, e registre a resposta — por exemplo, numa coluna "CPF/fidelidade informado" na tabela de lançamentos, ou entre parênteses no item/origem quando for relevante para uma comparação específica. Não presuma a partir do campo "identificado"/"não identificado" da nota.

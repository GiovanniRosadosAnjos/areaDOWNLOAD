# Inflação Pessoal — Registro de Gastos

**Objetivo:** acompanhar os gastos reais ao longo do ano (notas fiscais por categoria) para calcular, ano a ano, a inflação pessoal aplicada ao meu estilo de vida, em comparação com o índice oficial (IPCA/INPC).

Este arquivo é o registro mestre (editado diretamente, com histórico de versões pelo Git). A cópia no Google Drive é sincronizada a partir daqui, quando solicitado.

## Categorias (inicial, ajustável)
- Alimentação
- Moradia
- Transporte
- Saúde
- Lazer
- Assinaturas/Serviços digitais
- Outros

## 2026

### Setembro
| Data | Categoria | Descrição | Valor (R$) |
|------|-----------|-----------|------------|
| 05/09 | Alimentação | Supermercado Dalben (cebola, limão, carne, frango a passarinho) | 51,71 |
